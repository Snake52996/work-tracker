...
本项目是用于开发WT的源码库。无论你对WT感到有趣、困惑还是质疑，这里的源码都会为您保持开放，等待您的检查。当然，在多数情况下，您可以直接访问此网址([this first-party URL](https://snake.moe/wt/))，而无需下载整个源码库。

## Work Tracker 是什么？

Work Tracker是一个前端网页式工具，用于记录您品鉴过的作品，并且它只在您自己的电脑上运行。
我们能品鉴的作品浩如烟海，但我们有时却会忘记自己品鉴过某部作品，或者忘记自己对某个作品的评价是什么。一些优秀的作品让人看后深受触动，心生敬仰，一些人会通过写文章的方式，专门记录下自己的感受和反应。然而，想对每个作品都撰文记录既耗时又耗力，而且我们有时候很难给出“好”或是“坏”这样极端的评价。
...
**注意**
虽然WT功能全面，而且我们每天都在使用它，但是它并没有经过彻底的检验，您需要自行承担可能的风险。我们**强烈推荐**您**备份**您的宝贵数据。如果您发现WT存在任何问题，欢迎您通过Issues向我们报告，或者通过PR来贡献您的力量，我们会竭诚为您服务。

## 特征

- **仅有前端**：WT完全在您自己的设备上运行，不依赖任何特定服务器，这从根源上减少了数据迁移不便和被他人窥视的可能性。您的数据只属于您，仅此而已。
- **固有安全性**：在WT设计之初，我们就将如何应对非法访问，保障数据安全置于至关重要的地位。我们用很强的机密算法保护您的数据库，并且十分审慎地去选取新的依赖项：避免不必要的依赖，并且更偏爱那些被频繁使用，广泛测试，积极更新的依赖。当然，要想使我们的设计得以发挥，您需要阅读[security concerns](#security-concern)。
- **灵活机动**：您可以在创建您的数据库的时候，设计其中每个条目就有的属性。我们提供三种属性来实现您的需求，这些属性是可有可无的，并且可以按照您喜欢的方式排序。
...
- **Sync ready**WT作为仅有前端的应用，不提供实现多端同步的内置服务。不过，您可以用一个URL来加载数据库，这一数据库经过加密，可以很安全地上传网络，这意味着您可以把它放在一个文件存储平台上，用一个已知的、直接的链接来实现多端同步。此应用也可以在任何同步设备上工作，因为数据库中的文件修改时间窗口是可预测的，且完全由您掌握。相关建议请阅读[security concerns](#security-concern)。

## 安全性相关

本应用专为安全性设计，且由强大的现代加密算法赋能，但为了让这些设计能正常工作，还是需要您特别留意。请查看一下建议。
...
- **使用高强度的密码**:除了数据库以外，任何人要想访问数据都必须使用密码。如果您只是把密码简单地设置为`123456`，那无论多强的加密算法都保护不了您的数据安全。您的密码应该足够长、包含多种字符并且难以被猜到，尤其在实现多端同步时。
...
WT注入器已被构建，您可以查阅[this first-party URL](https://snake.moe/wt-injector/)中的说明来获取它，这份说明还将在WT注射器安装好后充当配置页。数据库和脚本可以被分开配置，其中数据库只能用URL进行导入，而脚本既可以用URL导入，也可以被直接存放在配置中。所谓“配置”，在这里是指一个网页，或者说是一个主机，它由正则表达式匹配，并且是您数据库和脚本的注入对象。
您的脚本可以是经典模式的也可以是模块形式的，前者用eval()来加载，后者用import()来导入。您需要根据不同形式的脚本，选择返回或输出以下对应的接口，以便注射器能正常工作。
...
其中，`run`负责运行脚本，`stop`负责终端脚本，`reset_data`则能在不重启脚本的情况下，用更快的方法替换您脚本中提供的数据库。`stop`和`reset_data`都是非必要提供的。没有`reset_data`的情况下，脚本或数据库的更新通过`stop`和`run`实现；如果同时没有`reset_data`和`stop`，那就只能通过重新加载网络来实现更新了。您可以参考以下代码片段来理解如何使您的注入器可交互。
...
本节写给想建立本应用实例或者参与开发的人。

### 建立实例

本应用使用前端开发的常用工作流。我们的包管理器为[bun](https://bun.sh/#getting-started)，不过您也可以用自己喜欢的。请使用`bun install`来安装依赖，在开发前请确保您的环境已经配置完全。
有三个配置文件用于建立此项目，第一个用于主应用，第二个用于注射器，第三个用于注射器配置页。您可以使用以下脚本：

- `build:app`: 建立主应用
- `build:userscript`: 建立注射器
- `build:userscript-configure-page`: 建立配置页
- `build`: 建立以上所有

### 开发WT注射器

在您提交对WT注射器的修改（或者说对`src/extension`下的文件修改）之前，请遵从以下规范。

1. 除了对index.html， main.ts 和 ConfigurePage.vuef的修改以外，绝大多数情况下都需要更新UserScript的版本，您可以在entry.ts处设置其版本。

2. 每当对类InjectorConfiguration提供的接口进行重大更改时，您都需要在versions.json中给interface_version的值增加1。这确保了配置页面与它所依赖的后端用户脚本同步。
